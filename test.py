        ...
        for cnt, data_dict in enumerate(data_loader):
            ...
            for cat in dsets.category:
                ...
                for pred in result:
                    ...
                    if cat == 'Tooth':
                        ori_image = cv2.drawContours(ori_image, [np.int0(box)], -1, (0, 255, 0), 2, 1)
                        cv2.putText(ori_image, '{:.2f} {}'.format(score, cat), (int(box[1][0]), int(box[1][1])),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,255,255), 1,1)

                    elif cat == 'Restoration':
                        ori_image = cv2.drawContours(ori_image, [np.int0(box)], -1, (0, 0, 255), 2, 1)
                        cv2.putText(ori_image, '{:.2f} {}'.format(score, cat), (int(box[1][0]), int(box[1][1])),
                                    cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 255), 1, 1)

                    else:
                        ori_image = cv2.drawContours(ori_image, [np.int0(box)], -1, (255, 0, 0), 2, 1)
                        cv2.putText(ori_image, '{:.2f} {}'.format(score, cat), (int(box[1][0]), int(box[1][1])),
                                    cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 255), 1, 1)
            ...
            cv2.imwrite(os.path.join(output_image_path  , img_id + '.png') , ori_image)
